# Functions to make and use docker containers in this project
...
run:
	nvidia-docker run -it -p $(LOCAL_JUPYTER_PORT):8888 -p $(LOCAL_TENSORBOARD_PORT):6006 -v $(shell pwd):/tensorflow/workspace --runtime $(RUNTIME) --network $(NETWORK) --name $(CONTAINER) $(IMAGE)
...
build:
	docker buildx build --platform linux/arm64 --push -t $(IMAGE) ./dockerfiles/jetson
