...
if __name__ == "__main__":
    ...
    base_path = r'C:\Users\sdtul\Grip_stuffv2'
        ...
        while not(done):
            ...
            if (len(line)==0):
                ...
                if (False):
                    ...
                    # Convert the 1 extracted box into a list
                    for point in range(0,2):
                        rows.append(my_strip_nonnumber(elements[1+point*2+1]))
                        cols.append(my_strip_nonnumber(elements[1+point*2+0]))
                        
                UL_row = int(np.floor(np.min(rows)) - 4)
                LR_row = int(np.ceil(np.max(rows)) + 4)
                UL_col = int(np.floor(np.min(cols)) - 4)
                LR_col = int(np.ceil(np.max(cols)) + 4)
                ...
                threshold = minv + 0.57*(maxv-minv)

                if (debug_measure):
                    plt.figure(10)
                    plt.clf()
                    plt.imshow(debug_image,cmap='hot')
