...
class HubPipeline(VisionPipeline):
    ...
    def process(self):
        ...
        row, col = self.filterImage.processContours(img)

        # Extrapolate distance and angle from given 
        targetDistance = self.filterImage.estimate_target_distance(row, self.cfg.getHeight())
        targetAngle =  self.filterImage.estimate_target_angle(col, self.hfov, self.cfg.getWidth())
