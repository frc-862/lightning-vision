...
class Processing():
    ...
    def __init__(self) -> None:
        ...
        # Allocate empty list of contours
        self.contours = []
    ...
    def save_image(self):
        """
        Saves image to /tmp/ directory when called
        """
    ...
    def estimate_target_angle(self,row,col):
        ...
        estimate_angle = (self.fov_horiz/640)*(col-640/2)
    ...
    def return_contours(self, img):

        """
            Previously working code, current implementation is for faster processing.
        if (False):
            # Threshold on 'color' (initially just keep bright things...)
            # BGR not RGB
        ...
        # Convert image to gray, then filter for certain contours 
        cv2.cvtColor(img, cv2.COLOR_BGR2GRAY, dst=self.sum_layer)
        ...
        self.contours, self.hierarchy = cv2.findContours(self.im_erode, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
